...
def save_data(data):
    ...
    try:
        parsed_dict = {key: value for key, value in [el.split("=") for el in parse_data.split("&")]}
        parsed_dict["date"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")

        formatted_dict = {
            "date": parsed_dict["date"],
            "username": parsed_dict.get("username", ""),
            "message": parsed_dict.get("message", "")
        }
        db.messages.insert_one(formatted_dict)
